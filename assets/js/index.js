...
window.innerWidth < 1000 ? cardContainer.classList.add('flex-column') : cardContainer.classList.remove('flex-column');
window.innerWidth > 768 ? imgContainer.classList.remove('d-none') : imgContainer.classList.remove('d-none');
...
function createElements() { // Create custom elements
options = getOptions();
...
function createTag(option) { // Create and show selected item as tag
...
function enableItemSelection() { // Add click listener to the list items
...
function isTagSelected(val) { // If the item is already selected
...
function removeTag(val) { // Remove selected item
for (var child of inputContainer.children) {
if (! child.classList.contains('input-body') && child.firstChild.dataset.value == val) {
...
function setValues() { // Update element final values
for (var i = 0; i < options.length; i++) {
element.options[i].selected = options[i].selected
}

}
function getOptions() { // Map element options
return [... element.options].map((op) => {
return {value: op.value, label: op.label, selected: op.selected}
})
}
}


new MultiSelectTag('ville');
